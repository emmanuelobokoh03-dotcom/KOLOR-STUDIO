...
// =====================
// CONTRACT TEMPLATE IMPORTS — shared source of truth for contracts + quotes (Iter 153)
// =====================

import { CONTRACT_TEMPLATES, fillContractTemplate } from '../data/contractTemplates';

// Display-only labels for contract types (stays local — not shared with quotes.ts)
...
router.post('/leads/:leadId/contracts', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const filledContent = content || fillContractTemplate(template.content, {
