    ...
    <>
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-0 md:p-4" onClick={onClose} role="presentation">
        ...
        >
          ...
          {/* ═══ Three-button action bar — persistent across all tabs ═══ */}
          <div
          ...
          >
            ...
            >
              <ChatCircle className="w-3.5 h-3.5" aria-hidden="true" />
              Message
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {activeTab === 'overview' ? (
              <div className="grid grid-cols-1 md:grid-cols-[1fr_240px] h-full" data-testid="overview-split-panel">
                      ...
                      <div className="space-y-0">
                        ...
                        <div className="flex gap-3 relative">
                          ...
                          <div className="flex-1 flex items-start justify-between min-w-0">
                            ...
                            {lead.discoveryCallCompletedAt && (
                              <button onClick={() => setActiveTab('pipeline')} className="h-7 px-2.5 rounded-md text-[10px] font-semibold text-white bg-[#6C2EDB] hover:brightness-110 transition flex-shrink-0" data-testid="send-quote-after-discovery-btn">Send {lang.quote}</button>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                {/* ═══ Right Panel — Timeline (order-1 on mobile, order-2 on desktop) ═══ */}
                <div className="bg-[var(--surface-background)] p-4 overflow-y-auto order-1 md:order-2 md:border-l" style={{ borderColor: 'var(--border)' }} data-testid="relationship-timeline">
                  {/* Key facts — always visible */}
                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                    <StatusBadge status={lead.status} />
                    {lead.estimatedValue && (
                      <span className="text-[10px] font-semibold text-[var(--text-secondary)] bg-[var(--surface-background)] border border-[var(--border)] rounded-full px-2 py-0.5">
                        {currencySymbol}{lead.estimatedValue.toLocaleString()}
                      </span>
                    )}
                  </div>
                  <h4 className="text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--text-secondary)] mb-3">Recent</h4>
                  {loadingActivities ? (
                    <ActivitySkeleton />
                  ) : (
                    <div className="space-y-0">
              ...
              <div className="p-4 md:p-6 space-y-4 md:space-y-6 pb-36 md:pb-6">
                ...
                {/* Notes — previous notes above, sticky input below on mobile */}
                <div>
                  ...
                  <div className="space-y-2 mb-4">
                    {activities.filter(a => a.type === 'NOTE_ADDED').length === 0 ? (
                      <p className="text-xs text-[var(--text-tertiary)] text-center py-4">No notes yet</p>
                    ) : (
                      activities.filter(a => a.type === 'NOTE_ADDED').map(activity => (
                        <div key={activity.id} className="bg-[var(--surface-background)] rounded-lg p-3 border border-[var(--border)]">
                          <p className="text-xs text-text-primary whitespace-pre-wrap">{activity.description}</p>
                          <p className="text-[10px] text-[var(--text-tertiary)] mt-1.5">{formatTimeAgo(activity.createdAt)}</p>
                        </div>
                      ))
                    )}
                  </div>
              ...
              {/* Sticky note input — pinned to bottom on mobile, inline on desktop */}
              <div
                className="sticky bottom-0 md:relative px-4 md:px-6 pb-4 pt-3 md:pb-6"
                style={{
                  background: 'var(--surface-base, #FDFCFF)',
                  backdropFilter: 'blur(12px)',
                  WebkitBackdropFilter: 'blur(12px)',
                  borderTop: '0.5px solid var(--border)',
                }}
                data-testid="files-tab-sticky-note"
              >
                <textarea
                  value={newNote}
                  onChange={(e) => setNewNote(e.target.value)}
                  placeholder={`Add a note about ${lead.clientName.split(' ')[0]}...`}
                  className="w-full bg-[var(--surface-background)] text-text-primary text-xs placeholder:text-[var(--text-tertiary)] resize-none rounded-lg px-3 py-2.5 mb-2"
                  style={{ border: '0.5px solid var(--border-dark, var(--border))', minHeight: '64px' }}
                  rows={2}
                  data-testid="files-tab-note-input"
                />
                <div className="flex justify-end">
                  <button
                    ...
                    disabled={addingNote || !newNote.trim()}
                    className="h-7 px-3 rounded-md text-[10px] font-semibold text-white bg-[#6C2EDB] transition disabled:opacity-40 flex items-center gap-1.5"
                    data-testid="files-tab-save-note"
                  >
                    {addingNote ? <KolorSpinner size={12} color="white" /> : <FloppyDisk className="w-3 h-3" />}
                    Save note
                  </button>
                </div>
              </div>
