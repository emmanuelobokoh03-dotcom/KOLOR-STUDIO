    ...
    <div className="h-screen overflow-hidden grid md:grid-cols-[420px_1fr] grid-cols-1">
      ...
      <div className="hidden md:flex flex-col justify-between relative overflow-hidden" style={{ background: '#080612', padding: '40px 40px 36px' }}>
        ...
        <div className="relative z-10" style={{ fontSize: 11, color: 'rgba(255,255,255,0.22)' }}>
          ...
          <Link to="/login" style={{ color: 'rgba(167,139,250,0.7)' }}>Sign in →</Link>
        </div>
      </div>
      ...
      <div className="flex flex-col justify-center bg-surface-base overflow-y-auto" style={{ padding: 'clamp(24px, 4vh, 52px) clamp(24px, 4vw, 48px)' }}>
        <div className="max-w-[400px] w-full mx-auto">
          ...
          <h1 className="text-2xl font-extrabold tracking-[-0.02em] text-text-primary mb-1">Create your account</h1>
          <p className="text-[13px] text-text-secondary mb-5">
            Already have one?{' '}
            <Link to="/login" className="font-medium" style={{ color: '#6C2EDB' }}>Sign in →</Link>
          </p>

          {/* Progress */}
          <div className="flex gap-1.5 mb-6">
            <span className="flex-1 h-[3px] rounded-sm" style={{ background: '#6C2EDB' }} />
            <span className="flex-1 h-[3px] rounded-sm" style={{ background: 'rgba(108,46,219,0.25)' }} />
            <span className="flex-1 h-[3px] rounded-sm bg-light-200" />
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={loading} className="w-full rounded-[10px] text-white text-sm font-bold tracking-[0.01em] transition-all duration-fast active:scale-[0.97] mt-4" style={{ height: 46, background: loading ? 'rgba(108,46,219,0.7)' : '#6C2EDB', cursor: loading ? 'not-allowed' : 'pointer' }} onMouseEnter={e => { if (!loading) e.currentTarget.style.background = '#5522B8' }} onMouseLeave={e => { if (!loading) e.currentTarget.style.background = '#6C2EDB' }} data-testid="signup-submit">
