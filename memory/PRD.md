...
- **`Dashboard.tsx` deep-link URL params**: new `useEffect` on mount reads `?leadId=xxx&section=contracts`, calls `leadsApi.getOne(dlLeadId)`, sets `selectedLeadInitialTab = section` and opens the lead modal. Params are cleared from the URL via `setSearchParams(cleaned, { replace: true })`. Coexists with the older `?openLead=&openLeadTab=` channel. Enables external links from emails / notifications to land directly on, e.g., the Contracts tab of a specific lead.
- Build gates: frontend `tsc --noEmit` 0 errors · `npm run build` ✅ (6.58s). Dashboard chunk: 289.35 KB → 289.62 KB (+0.27 KB for the new useEffect — negligible).
- Local commit: `28873dd feat: image compression + Esc key + deep-link URL params` (3 files, +64/-5). ⚠️ **`git push` failed locally — no GitHub creds. Use "Save to Github" to push.**
